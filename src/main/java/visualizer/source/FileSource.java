...
public class FileSource implements ISource {
	...
	public HashMap<String, HashMap<Integer, Double>> getBoltPendings(String component) {
		...
		int nbTopologies = topologies.size();
		for(int i = 0; i < nbTopologies; i++){
			Path boltPending = Paths.get(this.datasetDirectories.get(i) + "/" + CAT_BOLT + "/" 
					 + component + "_" + BOLT_PENDING + "_" + this.rootDirectories.get(i) + ".csv");
			if(Files.exists(boltPending)){
				try {
					HashMap<Integer, Double> dataset = new HashMap<>();
					ArrayList<String> data = (ArrayList<String>) Files.readAllLines(boltPending, Charset.defaultCharset());
					for(int j = 1; j < data.size(); j++){
						String[] line = data.get(j).split(";");
						dataset.put(Integer.parseInt(line[0]), Double.parseDouble(line[1]));
					}
